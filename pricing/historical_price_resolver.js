...
export function resolveTransactionPricing(tx) {
  const fx = resolveUsdtKrwRate({ timestamp: tx.timestamp });
  const assetForPrimaryPrice = selectPrimaryAsset(tx);
  const price = resolveAssetPriceUsdt({ asset: assetForPrimaryPrice, timestamp: tx.timestamp, tx });

  const priceKrw = Number.isFinite(price.price_usdt) && Number.isFinite(fx.rate)
    ? price.price_usdt * fx.rate
    : null;

  const amountInKrw = computeAmountKrw(tx.amount_in, tx.asset_in, tx, fx.rate);
  const amountOutKrw = computeAmountKrw(tx.amount_out, tx.asset_out, tx, fx.rate);
  const feeKrw = computeAmountKrw(tx.fee, tx.fee_asset, tx, fx.rate);
  ...
  return {
    ...
    pricing_source: choosePricingSource(price.pricing_source, fx.source),
...
function computeAmountKrw(amount, asset, tx, fxRate) {
  ...
  const resolved = resolveAssetPriceUsdt({ asset: symbol, timestamp: tx.timestamp, tx });
  if (!Number.isFinite(resolved.price_usdt)) return null;
  return numericAmount * resolved.price_usdt * fxRate;
}

function choosePricingSource(priceSource, fxSource) {
  if (priceSource === PRICING_SOURCE_MISSING || fxSource === FX_RATE_MISSING) return PRICING_SOURCE_MISSING;
