...
export function normalizeEvmWalletActivity(rawActivities, walletRecord, { userId = "demo-user" } = {}) {
...
function normalizeOne(activity, walletRecord, userId, index) {
  ...
  const base = {
    ...
    source_name: `${walletRecord.chain}:${activity.provider || "evm-provider"}`,
    source_file: "",
    raw_row_index: null,
    ...
    fee_asset: activity.feeAsset || getNativeAsset(walletRecord.chain),
    price_usdt: null,
    price_krw: null,
    amount_in_krw: null,
    amount_out_krw: null,
  ...
  if (classification.event_type === "SWAP") {
    base.asset_out = String(activity.assetSentSymbol || "").toUpperCase();
    base.amount_out = toNumber(activity.amountSent);
    base.asset_in = String(activity.assetReceivedSymbol || "").toUpperCase();
    base.amount_in = toNumber(activity.amountReceived);
    return base;
  }

  if (classification.event_type === "APPROVAL") {
    base.asset_out = String(activity.assetSymbol || "").toUpperCase();
    base.amount_out = 0;
    base.asset_in = String(activity.assetSymbol || "").toUpperCase();
    base.amount_in = 0;
    return base;
  }
  ...
  if (classification.event_type === "TRANSFER_OUT") {
    ...
    base.asset_in = base.asset_out;
    base.amount_in = base.amount_out;
    return base;
  }

  base.asset_in = String(activity.assetReceivedSymbol || activity.assetSymbol || "UNKNOWN").toUpperCase();
  base.asset_out = String(activity.assetSentSymbol || activity.assetSymbol || "UNKNOWN").toUpperCase();
  base.amount_in = toNumber(activity.amountReceived);
...
function buildRawDescription(activity) {
  const safe = {
    ...
    assetSentSymbol: activity.assetSentSymbol,
    amountSent: activity.amountSent,
    assetReceivedSymbol: activity.assetReceivedSymbol,
    amountReceived: activity.amountReceived,
    rawLegs: activity.rawLegs,
  };
